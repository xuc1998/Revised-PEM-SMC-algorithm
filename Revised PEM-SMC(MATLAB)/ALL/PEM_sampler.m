...
output_LE=importdata('./output_LE.txt');
output_NEE=importdata('./output_NEE.txt');
observed_data=[output_LE,output_NEE];
...
%         alp_old=betas_new*target(parameter_new(i,:),noise_LE,noise_NEE);
%         alp_media=betas_new*target(parameter_median,noise_LE,noise_NEE);    
%          % M-H accept
...
%         alp_old=betas_new*(target(parameter_new(old_ind(1),:),noise_LE,noise_NEE)+target(parameter_new(old_ind(2),:),noise_LE,noise_NEE));               
%         alp_media=betas_new*(target(parameter_median(2*k-1,:),noise_LE,noise_NEE)+target(parameter_median(2*k,:),noise_LE,noise_NEE)); 
%         % M-H accept
%         ratio=min([1,exp(alp_media-alp_old)]);
%         u=rand;
%         if u<ratio            
...
%  
    ...
    disp(['Mission has been complished:' num2str(stage/S*100) '%']);
